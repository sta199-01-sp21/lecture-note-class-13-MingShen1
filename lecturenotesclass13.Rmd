...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-MingShen1/manhattan.csv")
```
...
**Interpretation**: We are 95% sure that the mean price of one-bedroom
apartments lies between $2299 and $2977
...
ggplot(data = boot_median, mapping = aes(x = stat)) +
  geom_histogram(binwidth = 50)
get_ci(boot_median, level = 0.95)
```

We are 95% confident that the mean of one-bedroom apartments in Manhattan is 
...
We are 95% confident that the proportion of mice that were not on treatment that
died is between 69% and 79.7%
...
mice_lived <- mice %>%
  filter(treat == 1) %>%
  mutate(died = if_else(died == 0,"lived", "died")) %>%
  specify(response = died, success = "lived") %>%
  generate(reps = 10000, type = "bootstrap") %>%
  calculate(stat = "prop")
```
...
We are 90% confident that the proportion of mice lived that were on treatment
is between 55.2% and 65.1%.
